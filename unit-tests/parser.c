...
const char* token4 = "{\"type\":\"Optional\",\"value\":{\"type\": \"Array\",\"value\":"
...
const char* token5 = "{\"type\":\"Optional\",\"value\":{\"type\": \"Array\",\"value\":"
...
const char* token7 = "{\"type\": \"Array\",\"value\":"
                     "[{\"type\":\"String\",\"value\":\"e845b8406e4f43f79d3c1d8cacb3d5f3e7aeedb29feaeb4559fdb71a97e2fd0438565310e87670035d83bc10fe67fe314dba5363c81654595d64884b1ecad1512a64e65e020164\"},"
                     "{\"type\":\"String\",\"value\":\"d845b8406e4f43f79d3c1d8cacb3d5f3e7aeedb29feaeb4559fdb71a97e2fd0438565310e87670035d83bc10fe67fe314dba5363c81654595d64884b1ecad1512a64e65e020164\"}]}";
...
void createArgList() {
    ...
    flow_argument_list_t new_arg_list = {context0, {context2, context3, context4, context5, context6, context7}, 6};
...
static void test_printArbitraryArgument(void **state) {
    ...
    parser_error_t err = parser_printArbitraryArgument(&arg_list, 0, outKeyBuf, 40, outValBuf, 40, 0, &pageCountVar);
    assert_int_equal(err, PARSER_OK);
    assert_int_equal(pageCountVar, 1);
    ...
    pageCountVar = 0;
    err = parser_printArbitraryArgument(&arg_list, 1, outKeyBuf, 40, outValBuf, 40, 0, &pageCountVar);                                                                  
    assert_int_equal(err, PARSER_OK);
    assert_int_equal(pageCountVar, 1);
    assert_string_equal (outKeyBuf, "2: Optional");
    assert_string_equal (outValBuf, "{\"type\":\"UFix64\",\"value\":\"545.77\"}");

    pageCountVar = 0;
    err = parser_printArbitraryArgument(&arg_list, 2, outKeyBuf, 40, outValBuf, 40, 0, &pageCountVar);                                                                  
    assert_int_equal(err, PARSER_OK);
    assert_string_equal (outKeyBuf, "3: Optional");
    assert_string_equal (outValBuf, "{\"type\": \"Array\",\"value\":[{\"type\":\"Stri");

    err = parser_printArbitraryArgument(&arg_list, 2, outKeyBuf, 40, outValBuf, 40, 1, &pageCountVar);                                                                  
    assert_int_equal(err, PARSER_OK);
    assert_string_equal (outKeyBuf, "3: Optional");
    assert_string_equal (outValBuf, "ng\",\"value\":\"f845b8406e4f43f79d3c1d8cac");

    pageCountVar = 0;
    err = parser_printArbitraryArgument(&arg_list, 3, outKeyBuf, 40, outValBuf, 40, 0, &pageCountVar);                                                                  
    assert_int_equal(err, PARSER_OK);
    assert_string_equal (outKeyBuf, "4: Optional");
    assert_string_equal (outValBuf, "<Value too long>");

    pageCountVar = 0;
    err = parser_printArbitraryArgument(&arg_list, 4, outKeyBuf, 40, outValBuf, 40, 0, &pageCountVar);                                                                  
    assert_int_equal(err, PARSER_OK);
    assert_string_equal (outKeyBuf, "5: UFix64");
    assert_string_equal (outValBuf, "545.77");

    pageCountVar = 0;
    err = parser_printArbitraryArgument(&arg_list, 5, outKeyBuf, 40, outValBuf, 40, 0, &pageCountVar);                                                                  
    assert_int_equal(err, PARSER_OK);
