...
static bool isElementChar(uint8_t scriptChar) {
    return ('a' <= scriptChar && scriptChar <= 'z') || ('A' <= scriptChar && scriptChar <= 'Z') ||
           ('0' <= scriptChar && scriptChar <= '9') || ('_' == scriptChar);
}

// returns parsed element length, 0 indicates error
static size_t parseElement(size_t index, const uint8_t *scriptToParse, size_t scriptToParseSize) {
...
bool parseScript(script_parsed_elements_t *parsedElements,
    ...
    while (templateRead < scriptTemplateSize) {
        ...
        if (templateChar == 1) {  // token to parse
            ...
            parsedElements->elements[parsedElements->elements_count].data =
                scriptToParse + scriptRead;
...
#define ELEMENTS_MUST_BE_EQUAL(pE, i, j)                                                \
    {                                                                                   \
        ...
        if (MEMCMP(pE->elements[i].data, pE->elements[j].data, pE->elements[i].length)) \
            return false;                                                               \
    }
...
#define ELEMENT_MUST_BE_ADDRESS(pE, i)                                                   \
    {                                                                                    \
        _Static_assert(                                                                  \
            sizeof(NONFUNGIBLETOKEN_METADATAVIEWS_TESTNET) == ADDRESS_STRING_LENGTH + 1, \
            "Incompatible types");                                                       \
        _Static_assert(                                                                  \
            sizeof(NONFUNGIBLETOKEN_METADATAVIEWS_MAINNET) == ADDRESS_STRING_LENGTH + 1, \
            "Incompatible types");                                                       \
        if (i >= pE->elements_count) return false;                                       \
        if (pE->elements[i].length != ADDRESS_STRING_LENGTH) return false;               \
        if (MEMCMP(pE->elements[i].data,                                                 \
                   NONFUNGIBLETOKEN_METADATAVIEWS_TESTNET,                               \
                   pE->elements[i].length) &&                                            \
            MEMCMP(pE->elements[i].data,                                                 \
                   NONFUNGIBLETOKEN_METADATAVIEWS_MAINNET,                               \
                   pE->elements[i].length))                                              \
            return false;                                                                \
    }
...
bool parseNFT1(script_parsed_elements_t *parsedElements,
    ...
    const char template[] =
        ...
        "    acct.link<&{NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, \001.\001, "
    ...
    if (!parseScript(parsedElements,
                     scriptToParse,
                     scriptToParseSize,
                     (const uint8_t *) template,
                     sizeof(template) - 1)) {  // -1 to strip terminating 0
        return false;
    }
    ...
    ELEMENTS_MUST_BE_EQUAL(parsedElements,
                           PARSED_ELEMENTS_NFT1_CONTRACT_NAME,
                           PARSED_ELEMENTS_NFT1_CONTRACT_NAME2);
    ELEMENTS_MUST_BE_EQUAL(parsedElements,
                           PARSED_ELEMENTS_NFT1_STORAGE_PATH,
                           PARSED_ELEMENTS_NFT1_STORAGE_PATH2);
    ELEMENTS_MUST_BE_EQUAL(parsedElements,
                           PARSED_ELEMENTS_NFT1_STORAGE_PATH,
                           PARSED_ELEMENTS_NFT1_STORAGE_PATH3);
...
bool parseNFT2(script_parsed_elements_t *parsedElements,
    ...
    ELEMENTS_MUST_BE_EQUAL(parsedElements,
                           PARSED_ELEMENTS_NFT2_CONTRACT_NAME,
                           PARSED_ELEMENTS_NFT2_CONTRACT_NAME2);
    ELEMENTS_MUST_BE_EQUAL(parsedElements,
                           PARSED_ELEMENTS_NFT2_CONTRACT_NAME,
                           PARSED_ELEMENTS_NFT2_CONTRACT_NAME3);
    ELEMENTS_MUST_BE_EQUAL(parsedElements,
                           PARSED_ELEMENTS_NFT2_STORAGE_PATH,
                           PARSED_ELEMENTS_NFT2_STORAGE_PATH2);
